...
# ─────────────────────────────────────────────────────────────────────────────
# MarketConditionFilter (Feature #2)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# OPTIMIZED: ProfitGuard Engine
# ─────────────────────────────────────────────────────────────────────────────

class ProfitGuard:
    """
    Advanced trailing profit-locking daemon with dynamic account normalization
    and atomic enforcement safety.

    Modifications from standard architecture:
    1. Dynamic Threshold Normalization: Rather than binding retracements to static
       nominal USD thresholds ($1, $5, $20), the guard evaluates peak profits
       relative to the account's lock-adjusted equity base. This resolves sizing
       brittleness across account tiers.
    2. Atomic Enforcement Locks: Modification and position termination loops execute
       inside explicit thread-safe guards (`_api_lock`), eliminating race conditions 
       against the broker interface.
    """

    ACTIVATE_NORM_PCT: float      = 0.0005  # Activate trailing at 0.05% of account equity
    BREAKEVEN_NORM_PCT: float     = 0.002   # Move to BE at 0.2% of account equity
    DAMAGE_CONTROL_NORM_PCT: float = 0.005  # Activate damage protection at 0.5% equity peak
    DAMAGE_LOSS_NORM_PCT: float   = 0.001   # Damage close triggered at -0.1% equity loss
    CHECK_INTERVAL: int           = 5

    # Tuned Dynamic Tiers: (Peak_Equity_Percentage_Ceiling, Retracement_Threshold)
    TIERS: List[Tuple[float, float]] = [
        (0.001,        0.70),  # Peak < 0.1% equity: allow 70% retrace
        (0.005,        0.55),  # Peak < 0.5% equity: allow 55% retrace
        (0.02,         0.45),  # Peak < 2.0% equity: allow 45% retrace
        (float("inf"), 0.35),  # Peak ≥ 2.0% equity: tighten to 35% retrace
    ]
    ...
    def _get_equity_base(self) -> float:
        """Safely retrieves account equity adjusted for lock constraints."""
        acc = self.broker.getAccountInfo()
        if not acc or acc.equity <= 0:
            return 10000.0  # Safe fallback baseline
